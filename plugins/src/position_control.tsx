...
const PositonControlPlugin : Component = () => {
	const [c1Setpoint, setC1Setpoint] = createSignal<number>(0);
	const [c1Position, setC1Position] = createSignal<number>(0);

	const [c2Setpoint, setC2Setpoint] = createSignal<number>(0);
	const [c2Position, setC2Position] = createSignal<number>(0);

	const [tableSetpoint, setTableSetpoint] = createSignal<number>(0);
	const [tablePosition, setTablePosition] = createSignal<number>(0);

	const [detectorSetpoint, setDetectorSetpoint] = createSignal<number>(0);
	const [detectorPosition, setDetectorPosition] = createSignal<number>(0);

	const [c1_x, setC1_x] = createSignal<Array<number>>([]);
	const [c1_y, setC1_y] = createSignal<Array<number>>([]);
	const [c2_x, setC2_x] = createSignal<Array<number>>([]);
	const [c2_y, setC2_y] = createSignal<Array<number>>([]);
	const [table_x, setTable_x] = createSignal<Array<number>>([]);
	const [table_y, setTable_y] = createSignal<Array<number>>([]);
	const [detector_x, setDetector_x] = createSignal<Array<number>>([]);
	const [detector_y, setDetector_y] = createSignal<Array<number>>([]);

	const rdb = new MxRdb();

	function writeSetpointC1(value: number) : void {
		setC1Setpoint(value);
		rdb.write('/user/xpsrld4/c1/setpoint', value);
	}

	function writeSetpointC2(value: number) : void {
		setC2Setpoint(value);
		rdb.write('/user/xpsrld4/c2/setpoint', value);
	}

	function writeSetpointTable(value: number) : void {
		setTableSetpoint(value);
		rdb.write('/user/xpsrld4/table/setpoint', value);
	}

	function writeSetpointDetector(value: number) : void {
		setDetectorSetpoint(value);
		rdb.write('/user/xpsrld4/detector/setpoint', value);
	}

	const npoints = 500;

	function addHistoryPos1(value: number) : void {
		setC1_y(y => { const ny = Array.from(y); ny.push(value); return ny.slice(-npoints); });
		setC1_x(x => { const nx = Array.from(x); nx.push(Date.now() / 1000); return nx.slice(-npoints); });
	}

	function addHistoryPos2(value: number) : void {
		setC2_y(y => { const ny = Array.from(y); ny.push(value); return ny.slice(-npoints); });
		setC2_x(x => { const nx = Array.from(x); nx.push(Date.now() / 1000); return nx.slice(-npoints); });
	}

	function addHistoryPosTable(value: number) : void {
		setTable_y(y => { const ny = Array.from(y); ny.push(value); return ny.slice(-npoints); });
		setTable_x(x => { const nx = Array.from(x); nx.push(Date.now() / 1000); return nx.slice(-npoints); });
	}

	function addHistoryPosDetector(value: number) : void {
		setDetector_y(y => { const ny = Array.from(y); ny.push(value); return ny.slice(-npoints); });
		setDetector_x(x => { const nx = Array.from(x); nx.push(Date.now() / 1000); return nx.slice(-npoints); });
	}
	...
	onMount(() => {
		...
		rdb.watch('/user/xpsrld4/table/position', (_: string, value: MxGenericType) => {
			const pos = value.astype('float64');
	...
	return (
		<Card title="Position Control Panel">
			<div>
				<div class="flex gap-5 mb-5">
					<div class="flex gap-5">
						<MxValueControl title="C1 Setpoint" value={c1Setpoint()} size="xlarge" onChange={writeSetpointC1} increment={0.001} min={0} max={360} units="deg"/>
						<MxValuePanel title="C1 Position" value={c1Position().toFixed(5)} size="xlarge" units="deg" reactive/>
					</div>
					<MxGenericPlot
						series={[{}, { label: 'C1 Position', stroke: 'blue' }]}
						x={c1_x()}
						y={[c1_y()]}
						scales={{ x: { time: true }}}
						dimensions={[1000, 180]}
				...
				<div class="flex gap-5 mb-5">
					<div class="flex gap-5">
						<MxValueControl title="C2 Setpoint" value={c2Setpoint()} size="xlarge" onChange={writeSetpointC2} increment={0.001} min={0} max={360} units="deg"/>
						<MxValuePanel title="C2 Position" value={c2Position().toFixed(5)} size="xlarge" units="deg" reactive/>
					</div>
					<MxGenericPlot
						series={[{}, { label: 'C2 Position', stroke: 'blue' }]}
						x={c2_x()}
						y={[c2_y()]}
						scales={{ x: { time: true }}}
						dimensions={[1000, 180]}
				...
				<div class="flex gap-5 mb-5">
					<div class="flex gap-5">
						<MxValueControl title="TA Setpoint" value={tableSetpoint()} size="xlarge" onChange={writeSetpointTable} increment={0.001} min={0} max={360} units="deg"/>
						<MxValuePanel title="TA Position" value={tablePosition().toFixed(5)} size="xlarge" units="deg" reactive/>
					</div>
					<MxGenericPlot
						series={[{}, { label: 'Table Position', stroke: 'blue' }]}
						x={table_x()}
						y={[table_y()]}
						scales={{ x: { time: true }}}
						dimensions={[1000, 180]}
				...
				<div class="flex gap-5 mb-5">
					<div class="flex gap-5">
						<MxValueControl title="DE Setpoint" value={detectorSetpoint()} size="xlarge" onChange={writeSetpointDetector} increment={0.001} min={0} max={360} units="deg"/>
						<MxValuePanel title="DE Position" value={detectorPosition().toFixed(5)} size="xlarge" units="deg" reactive/>
					</div>
					<MxGenericPlot
						series={[{}, { label: 'Detector Position', stroke: 'blue' }]}
						x={detector_x()}
						y={[detector_y()]}
						scales={{ x: { time: true }}}
						dimensions={[1000, 180]}
...
export const pname  = 'Position Control';
export const render = PositonControlPlugin;
export const icon   = () => {
	return (
		<svg fill="#000000" width="120px" height="100px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
